...
/* @var $bidang app\models\BidangInstitut */
...
<div class="tab-content">
    ...
    <?php 
    //print "id_bidang = ".$bidang->id_bidang." id_agensi_institut : ".$bidang->id_agensi_institut."<br>";
/*
...
//*/
    ...
    //if($bil_personal!=0){//elak error jika tiada rekod personal dalam bidang institut

    //print "Bil Personal = ".$bil_personal."<br>";
    //row header
    
    //cari abiliti dalam bidang....ganti cari abiliti dalm PM
    /*
    $abiliti = BidangAbiliti::find()
    ...
    ->orderBy(['bidang_abiliti.id_bidang_abiliti' => SORT_DESC])//komen in utk atas abiliti terbaru - sama mcm penilaian
    ->all();
    */
    	...
    	->where(['bidang.id_bidang' => $bidang->id_bidang])
    	->andWhere('personal_bidang.is_aktif=1')
    	//->orderBy(['penilaian_profil.id_penilaian_profil' => SORT_DESC])//maksudnya disini ambil profil penilaian yg latest
    	->one();
    	//left koint hanya join table kedua shj
    ...
    if($bil_personal!=0&&$bil_cari_pp>0){//elak error jika tiada rekod personal dalam bidang institut
    	...
    	//->select('bidang_abiliti.*, bidang_duti.*')
	...
	*/
    ...
    foreach ($query as $pb){
    	...
    	//$namanic = $person->nama."<br>(KP : ".$person->no_kp.", U : ".$umur.", P : ".$pengalaman.")";
    	$ability_map[0][$col]=$namanic;	//mula [0][3]
    	//$nama->nama akan jadi array [0][2+...]
    	//echo "ID Personal Bidang : ".$pb['id_personal_bidang']." ID Personal : ".$pb['id_personal']." Nama :  ".$ability_map[0][$col]." Bidang : ".$bidang->id_bidang." <br>";
		...
		*/	
    	...
    	if($bil_pm==0){
    		//echo "Tidak membuat penilaian<br>";
    		for($k=1;$k<=$bil_abiliti;$k++){
    			...
    			foreach($pm as $pen_markah){
    				...
    				/*
    				if(!isset($pen_markah['markah_supervisor'])){
    					$ability_map[$row][$col]=1;
    				}else{
...
<?php 
//JADI Alhamdulillah...simpan dulu
//$ability_map=array_shift($ability_map);
?>
...
<table class="table table-striped table-bordered table-hover table-condensed">
	<thead>
		<tr><th class="col-md-2">Bil Personal</th><th class="col-md-2 text-center"><?=$bil_respondan;?></th></tr>
...
<?= ChartJs::widget([
	...
	'options'=>[
		'tooltipTemplate'=>  [$skor45, $skor4045, $skor3040,$skor3],	
		'showTooltips'=> 'true'
	]
]);
?>
...
<table class="table table-striped table-bordered table-hover table-condensed">
	<thead>
		<tr><th class="col-md-2">Bil Abiliti</th><th class="col-md-2 text-center"><?=$bil_abiliti;?></th></tr>
